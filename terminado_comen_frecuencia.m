function terminado_comen_frecuencia          
clc; close all;                    
...
function InicioBloques(~,~)         
    ...
    if isempty(x), return; end       % Si no hay señal cargada, salir

    blockSize = 4*Fs;                
    isPlaying = true;                % STOP

    for k = 1:floor(length(x)/blockSize)   % Recorre la señal bloque por bloque

        fprintf('Numero de bloque: %d\n', k); 
        if ~isPlaying, break; end            % STOP
        
        xb = x((k-1)*blockSize+1:k*blockSize); % Extrae el bloque k de la señal
        xb = applyEQ(xb);                      % Aplica ecualización FIR al bloque

        factor  = round(get(sliderML,'Value')); % Lee el factor M (decim) o L (interp)
        modeVal = get(modeMenu,'Value');       % 1 = Decimación, 2 = Interpolación
        
        h = SRC_fir(factor,401);               % Filtro pasabajo SRC (antialias / anti-imaging)

       %% ====== CONVOLUCIÓN EN FRECUENCIA ======
        Nfft = length(xb) + length(h) - 1;     % Tamaño FFT para convolución lineal
        
        Xb = fft(xb, Nfft);                    % FFT del bloque de señal
        H  = fft(h(:), Nfft);                  % FFT del filtro (forzado a columna)
        
        % ---------- DECIMACIÓN ----------
        if modeVal == 1
        
            Yb = ifft(Xb .* H);                % Multiplicación espectral = convolución
            yb = real(Yb(1:length(xb)));       % Recorta parte válida (longitud original)
            yb = yb(1:factor:end);             % Submuestreo (↓M)
            Fs2 = Fs/factor;                   % Nueva frecuencia de muestreo
        
        % ---------- INTERPOLACIÓN ----------
        else
        
            xu = zeros(length(xb)*factor,1);   % Vector para upsampling
            xu(1:factor:end) = xb;             % Inserta ceros (↑L)
        
            Nfft2 = length(xu) + length(h) - 1; % FFT size para nueva longitud
            Xb = fft(xu, Nfft2);                % FFT señal interpolada
            H  = fft(h(:), Nfft2);              % FFT del mismo filtro
        
            Yb = ifft(Xb .* H);                 % Filtrado anti-imaging
            yb = real(Yb(1:length(xu)));        % Recorte válido
            Fs2 = Fs*factor;                   % Nueva Fs
        end

        sound(yb,Fs2);                         % Reproduce el bloque procesado
        pause(blockSize/Fs);                   % Espera duración real del bloque
